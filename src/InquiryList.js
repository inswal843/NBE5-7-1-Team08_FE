  ...
  const fetchInquiries = (page = 0) => {
    fetch(`http://localhost:8080/inq/page?page=${page}&offset=10`)
      ...
      .catch((err) => {
        ...
        setAlertMessage("문의 목록을 불러오는데 실패했습니다!");  // 오류 메시지
      });
  };
    ...
    <div style={{
      ...
      <div className="container">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4" style={{ position: 'relative' }}>
          <h1 className="m-0 mx-auto text-center" style={{ flexGrow: 1 }}>문의 게시판</h1>
          <div className="d-flex align-items-center" style={{ position: 'absolute', right: 0 }}>
            <button className="btn btn-outline-secondary me-2" onClick={goToCreatePage}>문의하기</button>
          ...
          <table className="table table-striped table-hover table-sm">
            ...
            <tbody>
              {inquires.map((inquiry, index) => (
                <tr key={inquiry.inquiryId} style={{ cursor: "pointer" }} onClick={() => viewInquiry(inquiry.inquiryId)}>
                  <td style={{ textAlign: "center", whiteSpace: "nowrap" }}>{index + 1 + pageInfo.pageNumber * 10}</td>
                  <td style={{ textAlign: "center", whiteSpace: "nowrap" }}>{inquiry.title}</td>
                  <td style={{ textAlign: "center", whiteSpace: "nowrap" }}>{inquiry.name}</td>
                  <td style={{ textAlign: "center", whiteSpace: "nowrap" }}>{dayjs(inquiry.updatedAt).format('YYYY-MM-DD')}</td>
                  <td style={{ textAlign: "center", whiteSpace: "nowrap" }}>{dayjs(inquiry.createdAt).format('YYYY-MM-DD')}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <nav>
            <ul className="pagination" style={{
              ...
              {renderPageNumbers().map((page) => (
                <li key={page} className={`page-item ${pageInfo.pageNumber === page ? "active" : ""}`} style={{
                  borderRadius: "5px", // 버튼 모서리 둥글게
                }}>
                  <button className="page-link" style={{
                    color: pageInfo.pageNumber === page ? "#fff" : "#555",
                    backgroundColor: pageInfo.pageNumber === page ? "#333" : "transparent",
                    border: "1px solid #ddd", // 테두리 추가
                    borderRadius: "5px", // 버튼 모서리 둥글게
                    padding: "6px 12px", // 여백 추가
                    margin: "0 3px", // 버튼 사이 간격
                  }}
                    onMouseOver={(e) => { if (pageInfo.pageNumber !== page) { e.target.style.color = "#333"; } }}
                    onMouseOut={(e) => { if (pageInfo.pageNumber !== page) { e.target.style.color = "#555"; } }}
